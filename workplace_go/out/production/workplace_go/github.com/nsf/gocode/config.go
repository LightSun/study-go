...
type config struct {
	ProposeBuiltins    bool   `jsontest:"propose-builtins"`
	LibPath            string `jsontest:"lib-path"`
	CustomPkgPrefix    string `jsontest:"custom-pkg-prefix"`
	CustomVendorDir    string `jsontest:"custom-vendor-dir"`
	Autobuild          bool   `jsontest:"autobuild"`
	ForceDebugOutput   string `jsontest:"force-debug-output"`
	PackageLookupMode  string `jsontest:"package-lookup-mode"`
	CloseTimeout       int    `jsontest:"close-timeout"`
	UnimportedPackages bool   `jsontest:"unimported-packages"`
}
...
func (this *config) list() string {
	...
	for i := 0; i < str.NumField(); i++ {
		v := str.Field(i)
		name := typ.Field(i).Tag.Get("jsontest")
...
func (this *config) list_option(name string) string {
	...
	for i := 0; i < str.NumField(); i++ {
		v := str.Field(i)
		nm := typ.Field(i).Tag.Get("jsontest")
		if nm == name {
			list_value(v, name, buf)
		}
	}
